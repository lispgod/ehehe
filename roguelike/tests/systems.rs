...
// ─── Sandbox systems integration tests ───────────────────────────

/// Creates a minimal App wired for interaction/wanted/brawl testing.
fn test_app_sandbox() -> App {
    use roguelike::systems::{interaction, wanted, brawl, hiding};
    ...
    app.add_message::<AttackIntent>();
    app.add_message::<DamageEvent>();
    app.add_message::<InteractionIntent>();
    app.add_message::<CrimeEvent>();
    app.add_message::<HideIntent>();
    app.add_message::<BrawlEscalation>();
    app.init_resource::<SpatialIndex>();
    app.init_resource::<CombatLog>();
    app.init_resource::<KillCount>();
    app.init_resource::<SoundEvents>();
...
/// Spawns a player entity for sandbox tests.
fn spawn_sandbox_player(app: &mut App, x: i32, y: i32) -> Entity {
    app.world_mut().spawn((
        Position { x, y },
        Player,
        BlocksMovement,
...
// ─── Test: Wanted escalation on witnessed assault ────────────────

#[test]
fn crime_event_raises_wanted_level_when_witnessed() {
    ...
    // Emit a crime event for assault
    ...
    // Star level should have increased
    let star_level = app.world().resource::<StarLevel>();
    assert!(
        star_level.level > 0,
        "Witnessed assault should raise wanted level, got level={}",
        star_level.level
    );
}

#[test]
fn crime_event_does_not_raise_wanted_without_witness() {
    let mut app = test_app_sandbox();
    let player = spawn_sandbox_player(&mut app, 60, 40);

    // No NPCs nearby — crime is unwitnessed
    app.update();

    app.world_mut().write_message(CrimeEvent {
        perpetrator: player,
        crime: CrimeType::Murder,
    ...
    assert_eq!(
        star_level.level, 0,
        "Unwitnessed crime should not raise wanted level"
    );
}

#[test]
fn murder_raises_wanted_more_than_assault() {
    use std::collections::HashSet;

    // Test assault
    let mut app1 = test_app_sandbox();
    let player1 = spawn_sandbox_player(&mut app1, 60, 40);
    let crime_pos = GridVec::new(60, 40);
    let mut visible = HashSet::new();
    visible.insert(crime_pos);
    ...
    app1.update();
    app1.world_mut().write_message(CrimeEvent {
        perpetrator: player1,
        crime: CrimeType::Assault,
        position: crime_pos,
    });
    app1.update();
    let assault_level = app1.world().resource::<StarLevel>().level;

    // Test murder
    let mut app2 = test_app_sandbox();
    let player2 = spawn_sandbox_player(&mut app2, 60, 40);
    ...
    app2.update();
    app2.world_mut().write_message(CrimeEvent {
        perpetrator: player2,
        crime: CrimeType::Murder,
        position: crime_pos,
    });
    app2.update();
    let murder_level = app2.world().resource::<StarLevel>().level;

    assert!(
        murder_level > assault_level,
        "Murder (level={murder_level}) should raise wanted more than assault (level={assault_level})"
    );
}

// ─── Test: Hiding breaks NPC pursuit ─────────────────────────────

#[test]
fn hiding_sets_player_hidden_flag() {
    use roguelike::systems::hiding;
    ...
    // Create a map with a barrel at the player's position
    let mut map = GameMap::new(120, 80, 42);
    // Place a barrel at a known empty position
    let test_pos = GridVec::new(60, 40);
    if let Some(voxel) = map.get_voxel_at_mut(&test_pos) {
        voxel.props = Some(Props::Barrel);
    }
    app.insert_resource(GameMapResource(map));
    app.insert_resource(MapSeed(42));

    app.add_systems(Update, hiding::hide_system);
    ...
    // Emit hide intent
    app.world_mut().write_message(HideIntent {
        entity: player,
        entering: true,
    });
    app.update();

    // Player should now have Hidden component
    let has_hidden = app.world().entity(player).get::<Hidden>().is_some();
    assert!(has_hidden, "Player should have Hidden component after entering barrel");

    // Star level should show player as hidden
    let star_level = app.world().resource::<StarLevel>();
    assert!(star_level.player_hidden, "StarLevel should track player as hidden");
}

#[test]
fn exiting_hiding_creates_noise() {
    use roguelike::systems::hiding;
    ...
    let player = app.world_mut().spawn((
        Position { x: 60, y: 40 },
        Player,
        Hidden { hiding_pos: GridVec::new(60, 40) },
        Renderable {
            symbol: "0".into(), // Barrel appearance while hidden
            fg: ratatui::style::Color::White,
            bg: ratatui::style::Color::Black,
        },
    )).id();

    // Mark as hidden in star level
    app.world_mut().resource_mut::<StarLevel>().player_hidden = true;
    ...
    // Player should no longer have Hidden component
    let has_hidden = app.world().entity(player).get::<Hidden>().is_some();
    assert!(!has_hidden, "Player should lose Hidden component after exiting");

    // Sound event should have been created
    let sound_events = app.world().resource::<SoundEvents>();
    assert!(
        !sound_events.events.is_empty(),
        "Exiting hiding should generate a noise event"
    );

    // Player appearance should be restored
    let renderable = app.world().entity(player).get::<Renderable>().unwrap();
    assert_eq!(renderable.symbol, "@", "Player symbol should be restored to @");
}

// ─── Test: Fire chain-ignition ───────────────────────────────────

#[test]
fn fire_spreads_to_adjacent_flammable_props() {
    use roguelike::systems::turn;
    ...
    // Create a map with adjacent flammable props (haybale next to fire)
    let mut map = GameMap::new(120, 80, 42);
    let fire_pos = GridVec::new(60, 40);
    let hay_pos = GridVec::new(61, 40);

    // Set fire tile
    if let Some(voxel) = map.get_voxel_at_mut(&fire_pos) {
        voxel.floor = Some(roguelike::typeenums::Floor::Fire);
    }
    map.fire_turns.insert(fire_pos, 1);

    // Place haybale adjacent
    if let Some(voxel) = map.get_voxel_at_mut(&hay_pos) {
        voxel.props = Some(Props::HayBale);
    }

    app.insert_resource(GameMapResource(map));
    app.insert_resource(MapSeed(42));
    app.insert_resource(TurnCounter(8)); // Fire spreads every 8 turns

    app.add_systems(Update, turn::fire_system);
    ...
    // At minimum, the fire_turns map should be tracking fire positions
    assert!(
        game_map.0.fire_turns.contains_key(&fire_pos),
        "Original fire position should still be tracked"
    );
}

#[test]
fn barrel_is_explosive() {
    assert!(Props::Barrel.is_explosive());
    assert!(!Props::HayBale.is_explosive());
    assert!(!Props::Wall.is_explosive());
    assert!(!Props::Chair.is_explosive());
}

#[test]
fn barrel_is_also_hiding_spot() {
    assert!(Props::Barrel.is_hiding_spot());
    assert!(Props::HayBale.is_hiding_spot());
    assert!(Props::Outhouse.is_hiding_spot());
    assert!(Props::Wardrobe.is_hiding_spot());
    assert!(!Props::Wall.is_hiding_spot());
    assert!(!Props::Chair.is_hiding_spot());
    assert!(!Props::Table.is_hiding_spot());
}

#[test]
fn outhouse_and_wardrobe_are_flammable() {
    assert!(Props::Outhouse.is_flammable());
    assert!(Props::Wardrobe.is_flammable());
}

// ─── Test: Brawl escalation on weapon draw ───────────────────────

#[test]
fn brawl_escalation_alerts_law_npcs() {
    let mut app = test_app_sandbox();
    let player = spawn_sandbox_player(&mut app, 60, 40);

    // Spawn a sheriff NPC nearby
    let sheriff = app.world_mut().spawn((
        Position { x: 62, y: 40 },
        Name("Sheriff".into()),
        Health { current: 100, max: 100 },
        CombatStats { attack: 8 },
    ...
    // Emit brawl escalation (player draws weapon)
    app.world_mut().write_message(BrawlEscalation {
        entity: player,
        position: GridVec::new(60, 40),
    });
    app.update();

    // Sheriff should now be hostile
    let is_hostile = app.world().entity(sheriff).get::<Hostile>().is_some();
    assert!(
        is_hostile,
        "Sheriff should become hostile when weapon is drawn in a brawl"
    );
}

#[test]
fn brawl_escalation_emits_crime_event() {
    let mut app = test_app_sandbox();
    let player = spawn_sandbox_player(&mut app, 60, 40);

    // Spawn a civilian witness with LOS
    use std::collections::HashSet;
    let crime_pos = GridVec::new(60, 40);
    let mut visible = HashSet::new();
    visible.insert(crime_pos);
    ...
    // Emit brawl escalation — should generate a crime event
    app.world_mut().write_message(BrawlEscalation {
        entity: player,
        position: crime_pos,
    });
    app.update();
    // Second update: the CrimeEvent emitted by brawl_escalation_system is now
    // processed by crime_system in this tick.
    app.update();

    // The crime system processes CrimeEvent after brawl_escalation_system
    // So wanted level should increase due to the witnessed crime
    let star_level = app.world().resource::<StarLevel>();
    assert!(
        star_level.level > 0,
        "Brawl escalation should trigger a crime event that raises wanted level when witnessed"
    );
}

#[test]
fn brawl_witness_joins_same_faction() {
    let mut app = test_app_sandbox();
    ...
    // Spawn a nearby same-faction NPC (not yet in brawl)
    let bystander = app.world_mut().spawn((
        Position { x: 63, y: 40 },
        Name("Bystander".into()),
        Health { current: 60, max: 60 },
        CombatStats { attack: 3 },
    ...
    // Spawn a different-faction NPC (should not join)
    ...
    app.update();
    app.update(); // Run brawl witness system

    // Same-faction bystander should have joined the brawl
    let bystander_in_brawl = app.world().entity(bystander).get::<InBrawl>().is_some();
    assert!(
        bystander_in_brawl,
        "Same-faction NPC should join a nearby brawl"
    );

    // Different-faction NPC should NOT have joined
    let other_in_brawl = app.world().entity(other).get::<InBrawl>().is_some();
    assert!(
        !other_in_brawl,
        "Different-faction NPC should not join a brawl"
    );
}

// ─── Test: NPC Interaction hostility system ──────────────────────

#[test]
fn taunt_raises_npc_hostility() {
    let mut app = test_app_sandbox();
    let player = spawn_sandbox_player(&mut app, 60, 40);
    ...
    app.world_mut().write_message(InteractionIntent {
        player,
        target: npc,
        interaction: NpcInteraction::Taunt,
    });
    app.update();

    let hostility = app.world().entity(npc).get::<Hostility>().unwrap();
    assert!(
        hostility.level > 0,
        "Taunting should raise NPC hostility, got level={}",
        hostility.level
    );
}

#[test]
fn greeting_lowers_npc_hostility() {
    let mut app = test_app_sandbox();
    let player = spawn_sandbox_player(&mut app, 60, 40);
    ...
    app.world_mut().write_message(InteractionIntent {
        player,
        target: npc,
        interaction: NpcInteraction::Greet,
    });
    app.update();
    ...
    assert!(
        hostility.level < 20,
        "Greeting should lower NPC hostility, got level={}",
        hostility.level
    );
}

#[test]
fn repeated_taunts_make_npc_hostile() {
    let mut app = test_app_sandbox();
    let player = spawn_sandbox_player(&mut app, 60, 40);
    ...
    // Taunt multiple times to exceed calm threshold (100)
    for _ in 0..4 {
        app.world_mut().write_message(InteractionIntent {
            player,
            target: npc,
    ...
    let is_hostile = app.world().entity(npc).get::<Hostile>().is_some();
    assert!(
        is_hostile,
        "NPC should become hostile after repeated taunts"
    );
}

#[test]
fn drunk_npc_has_lower_hostility_threshold() {
    // A drunk NPC should become hostile faster than a calm one
    let mut app = test_app_sandbox();
    let player = spawn_sandbox_player(&mut app, 60, 40);

    let drunk_npc = app.world_mut().spawn((
        Position { x: 61, y: 40 },
        Name("Whiskey".into()),
        Health { current: 60, max: 60 },
        CombatStats { attack: 2 },
    ...
    // Two taunts = 60 hostility, should exceed drunk threshold (40)
    for _ in 0..2 {
        app.world_mut().write_message(InteractionIntent {
            player,
            target: drunk_npc,
    ...
    let is_hostile = app.world().entity(drunk_npc).get::<Hostile>().is_some();
    assert!(
        is_hostile,
        "Drunk NPC should become hostile after just 2 taunts (threshold=40, delta=30×2=60)"
    );
}
...
#[test]
fn buy_drink_deducts_gold() {
    let mut app = test_app_sandbox();
    let player = spawn_sandbox_player(&mut app, 60, 40);

    let bartender = app.world_mut().spawn((
        Position { x: 61, y: 40 },
        Name("Barman".into()),
        Health { current: 60, max: 60 },
        CombatStats { attack: 2 },
    ...
    app.world_mut().write_message(InteractionIntent {
        player,
        target: bartender,
        interaction: NpcInteraction::BuyDrink,
    });
    app.update();

    let gold_after = app.world().resource::<Gold>().0;
    assert!(
        gold_after < gold_before,
        "Buying a drink should deduct gold (before={gold_before}, after={gold_after})"
    );
}

#[test]
fn gold_starts_at_twenty() {
